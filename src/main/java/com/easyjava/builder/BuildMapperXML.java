    ...
    public static void execute(TableInfo tableInfo) {
        ...
        try (OutputStream out = new FileOutputStream(
            ...
            for (FieldInfo fieldInfo : tableInfo.getFieldListExtend()) {
                String andWhere = "";
                if (ArrayUtils.contains(Constants.SQL_STRING_TYPE, fieldInfo.getSqlType())) {
                    andWhere = "and " + fieldInfo.getFieldName() + " like concat('%',#{" + fieldInfo.getPropertyName()
                            + "},'%')";
            ...
            if (autoIncrement != null) {
                bw.write("\t\t<selectKey keyProperty=\"bean."+autoIncrement.getPropertyName()+"\" resultType=\"Integer\" order=\"AFTER\">\r\n" + //
            ...
            for (Map.Entry<String, List<FieldInfo>> entry : keyIndexInfo.entrySet()) {
                ...
                bw.write("\t\tselect\r\n" + //
                                        "\t\t<include refid=\"base_column_list\"/>\r\n" + //
                                        "\t\tfrom "+tableInfo.getTableName()+" where "+paramName);
                bw.newLine();
                bw.write("\t</select>");
                bw.newLine();
                bw.newLine();
